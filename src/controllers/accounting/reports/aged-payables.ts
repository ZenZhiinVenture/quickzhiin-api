...
export const getAgedPayables = async (req: Request, res: Response, next: NextFunction) => {
  try {
    // Only fetch bills that are approved or partially paid (exclude Draft, Void, Paid)
    ...
    for (const bill of bills) {
      ...
      const totalCredited = bill.billCreditNote ? Number(bill.billCreditNote.amount) : 0;
      
      const outstandingBalance = Number(bill.total) - totalPaid - totalCredited;
      if (outstandingBalance <= 0) continue; // Skip fully paid just in case

      const contactIdStr = bill.contactId.toString();
      if (!reportData[contactIdStr]) {
        reportData[contactIdStr] = {
          contactId: contactIdStr,
          contactName: bill.contact.legalname,
          current: 0,
          days_1_30: 0,
          days_31_60: 0,
          days_61_90: 0,
      ...
      const targetDate = bill.dueDate || bill.date;
    ...
    logger.error('Error generating Aged Payables report:', error);
